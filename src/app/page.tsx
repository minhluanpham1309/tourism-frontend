...
import { TourismQuery, TourismLocation } from '@/types/tourism';
import {
  Search,
  MapPin,
  Navigation,
...
export default function HomePage() {
  ...
  // Phân loại 1 địa điểm là "chính xác" (khớp địa giới) hay chỉ "gợi ý liên quan" (vector search).
  const isExact = (loc: TourismLocation) => {
    const s = (loc.source || '').toLowerCase();
    if (
      s.startsWith('kg_ward_match') ||
      s.startsWith('kg_search') ||
      s.startsWith('administrative') ||
      s.startsWith('province')
    )
      return true;
    return loc.confidence >= 0.7;
  };

  const exactResults = results ? results.results.filter(isExact) : [];
  const relatedResults = results ? results.results.filter((r) => !isExact(r)) : [];

  const renderCard = (loc: TourismLocation, i: number) => (
    <div
      key={`${loc.name}-${i}`}
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h3 className="font-semibold text-slate-900 text-xl leading-snug">{loc.name}</h3>
          <p className="mt-1.5 text-base text-slate-600 inline-flex items-center gap-1.5">
            ...
            <span className="truncate">
              {[loc.ward, loc.province].filter(Boolean).join(', ') || 'miền Nam'}
            </span>
          </p>
        </div>
        <span
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 space-y-5">
        ...
        {results && !isLoading && (
          <>
            ...
            {/* TOURISM result cards — tách 2 block */}
            {!isAdminResult && results.results.length > 0 && (
              <div className="space-y-6">
                {/* Block 1: Kết quả chính xác */}
                {exactResults.length > 0 && (
                  <div>
                    <p className="flex items-center gap-2 mb-3 text-sm font-semibold text-emerald-700">
                    ...
                    <div className="grid sm:grid-cols-2 gap-4">
                      {exactResults.map((loc, i) => renderCard(loc, i))}
                    </div>
                  </div>
                )}

                {/* Block 2: Gợi ý liên quan */}
                {relatedResults.length > 0 && (
                  <div>
                    <p className="flex items-center gap-2 mb-3 text-sm font-semibold text-amber-700">
                    ...
                    <div className="grid sm:grid-cols-2 gap-4">
                      {relatedResults.map((loc, i) => renderCard(loc, i))}
                    </div>
                  </div>
                )}
              </div>
